...
class Dataset(NoiseAugmentation):
    def __init__(self,
                 clean_dataset,
                #  clean_dataset_limit,
        ...
        with open(expand_path(clean_dataset), "rb") as f:
            for line in f:
                self.clean_dataset_list.append(line.rstrip(b'\n').decode('latin-1'))
        ...
        if pre_load_clean_dataset:
            self.clean_dataset_list = self._preload_dataset(clean_dataset_list, remark="Clean Dataset")
    ...
    def __getitem__(self, item):
        audios = self.clean_dataset_list[item]
        clean_files = self.augment(samples=audios, sample_rate=self.sr)
        clean_y = load_wav(clean_files, sr=self.sr)
